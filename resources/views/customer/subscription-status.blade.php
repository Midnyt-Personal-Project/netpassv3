...
<div class="max-w-lg mx-auto space-y-6">
    ...
    @if($customer)
        @php($active = $customer->status === 'active' && $customer->expires_at?->isFuture())
        <section @class(['rounded-2xl border p-5 sm:p-6 shadow-xl text-center','border-emerald-500/30 bg-emerald-500/10'=>$active,'border-rose-500/30 bg-rose-500/10'=>!$active])>
            <i @class(['fa-solid text-3xl','fa-circle-check text-emerald-300'=>$active,'fa-circle-xmark text-rose-300'=>!$active])></i><h3 class="mt-3 text-xl font-black text-white">{{ $active ? 'Your access is active' : 'Your access has expired' }}</h3>
            ...
            @unless($active)<a href="{{ route('customer.portal', $location->slug) }}" class="mt-5 inline-block rounded-lg bg-indigo-600 px-4 py-2 text-sm font-bold text-white">Renew subscription</a>@endunless
        </section>
    @endif
</div>
@endsection
