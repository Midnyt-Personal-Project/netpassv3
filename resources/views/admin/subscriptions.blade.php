...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
    <section class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
        ...
        <form method="POST" action="{{ route('admin.subscriptions.create') }}" class="space-y-4">
            ...
            <div class="rounded-lg border border-indigo-500/20 bg-indigo-500/10 p-3 text-xs text-indigo-100"><i class="fa-solid fa-ticket mr-1"></i>A secure voucher is generated automatically. The customer uses this one voucher code for MikroTik access.</div>
    ...
    <section class="xl:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl overflow-hidden">
        ...
        <div class="overflow-x-auto"><table class="w-full text-left text-sm"><thead><tr class="border-b border-slate-800 text-slate-400 text-xs uppercase"><th class="pb-3">Customer</th><th class="pb-3">Package</th><th class="pb-3">Location</th><th class="pb-3">Access expires</th><th class="pb-3">Status</th><th class="pb-3">Amount</th></tr></thead><tbody class="divide-y divide-slate-800/50">@forelse($subscriptions as $subscription)<tr><td class="py-3"><p class="font-semibold text-white">{{ $subscription->customer?->username ?? 'Pending customer' }}</p><p class="text-xs text-slate-500">{{ $subscription->customer?->phone_number }}</p></td><td class="py-3 text-slate-300">{{ $subscription->package->name }}</td><td class="py-3 text-slate-300">{{ $subscription->location->name }}</td><td class="py-3 text-slate-300">{{ $subscription->customer?->expires_at?->format('M j, Y g:i A') ?? '—' }}</td><td class="py-3 text-xs font-bold {{ $subscription->customer?->status === 'active' ? 'text-emerald-400' : ($subscription->customer?->status === 'blocked' ? 'text-red-400' : 'text-slate-500') }}">{{ $subscription->customer?->status ?? 'N/A' }}</td><td class="py-3 text-emerald-400">{{ number_format($subscription->amount, 2) }} GHS</td><td class="py-3 space-x-2">@if($subscription->customer)<form method="POST" action="{{ route('admin.subscriptions.block', $subscription->customer->id) }}" style="display:inline"><button type="submit" class="bg-red-600 hover:bg-red-500 text-white text-xs px-2 py-1 rounded">Block</button></form><form method="POST" action="{{ route('admin.subscriptions.remove', $subscription->customer->id) }}" style="display:inline"><button type="submit" class="bg-slate-600 hover:bg-slate-500 text-white text-xs px-2 py-1 rounded">Remove</button></form>@endif</td></tr>{{ $subscriptions->links() }}
@endempty<tr><td colspan="5" class="py-10 text-center text-slate-500">No completed subscriptions yet.</td></tr>@endforelse</tbody></table></div>
    </section>
</div>
@endsection
...
@endsection
<form method="GET" class="mb-4"><input name="q" placeholder="Search subscriptions..." value="{{ request("q") }}" class="bg-slate-800 border border-slate-700 rounded p-2 text-white"></form>
