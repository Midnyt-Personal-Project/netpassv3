...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
    ...
    <section class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
        ...
        <form method="POST" action="{{ route('admin.subscriptions.create') }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="location_id" required class="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white text-sm">
            ...
            <div>
                <label class="block text-xs text-slate-400 mb-1">Internet package</label>
                <select name="package_id" required class="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white text-sm">
                    @foreach($packages as $package)
                        <option value="{{ $package->id }}" data-location="{{ $package->location_id }}">
                            {{ $package->location->name }} — {{ $package->name }} ({{ number_format($package->price, 2) }} GHS)
                        </option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-xs text-slate-400 mb-1">Customer phone number</label>
                <input name="phone_number" required value="{{ old('phone_number') }}" placeholder="e.g. 0244123456" class="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white text-sm">
            </div>
            <div class="rounded-lg border border-indigo-500/20 bg-indigo-500/10 p-3 text-xs text-indigo-100">
                <i class="fa-solid fa-ticket mr-1"></i>A secure voucher is generated automatically. The customer uses this one voucher code for MikroTik access.
            </div>
            <details class="rounded-xl border border-slate-700 bg-slate-800/50 p-3">
                <summary class="cursor-pointer text-sm font-semibold text-indigo-300">
                    <i class="fa-solid fa-tv mr-1"></i>Add a smart device now <span class="text-slate-500 font-normal">(optional)</span>
                </summary>
                <p class="mt-2 text-xs text-slate-400">The device will be registered to the customer and granted access when the router syncs.</p>
                <div class="mt-3 space-y-3">
                    <div>
                        <label class="block text-xs text-slate-400 mb-1">Device name</label>
                        <input name="device_name" value="{{ old('device_name') }}" maxlength="50" placeholder="e.g. Living Room TV" class="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white text-sm">
                    </div>
                    <div>
                        <label class="block text-xs text-slate-400 mb-1">MAC address</label>
                        <input name="mac_address" value="{{ old('mac_address') }}" placeholder="AA:BB:CC:DD:EE:FF" class="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white text-sm font-mono">
                    </div>
                </div>
            </details>
    ...
    <section class="xl:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl overflow-hidden">
        ...
        <form method="GET" class="mb-4">
            <input name="q" placeholder="Search subscriptions..." value="{{ request('q') }}" class="bg-slate-800 border border-slate-700 rounded p-2 text-white w-full">
        </form>
...
<script>
    ...
    function filterPackages() {
        ...
        const chosen = packageSelect.selectedOptions[0];
        if (!chosen || chosen.hidden) {
            packageSelect.value = [...packageSelect.options].find(option => !option.hidden)?.value || '';
        }
    }
