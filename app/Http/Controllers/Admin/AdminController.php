...
class AdminController extends Controller
{
    ...
    )
    {
        ...
        // Voucher SMS is sent immediately through the ONE shared function used
        // by every purchase path (manual admin sale AND customer online sale):
        // SmsService::sendCredentials() → direct Arkesel URL call, no queue.
    ...
    /**
     * Send the voucher SMS by calling the Arkesel API URL directly — the exact
     * same request as opening this in a browser:
     *
     *   https://sms.arkesel.com/sms/api?action=send-sms
     *       &api_key=YOUR_KEY&to=0244xxxxxxx&from=SenderID&sms=Message
     *
     * The key comes from ARKESEL_SMS_API_KEY in .env. The number is tried in
     * local format first (0542...) and automatically retried as 23354... if
     * the gateway rejects it. The real outcome is always recorded.
     */
    protected function sendVoucherSmsDirect(Customer $customer, string $packageName): bool
    {
        $apiKey = (string) config('services.arkesel.api_key');
        $senderId = (string) config('services.arkesel.sender_id', 'OyaloWiFi');
        ...
        if (blank($apiKey)) {
            Log::error('[SMS] Not sent — ARKESEL_SMS_API_KEY is not configured in .env.', [
                'type' => SmsLog::TYPE_VOUCHER,
                'phone' => $customer->phone_number,
                'customer' => $customer->voucher_code,
            ]);

            return false;
        }

        // Local format first, international fallback.
        $candidates = PhoneNumber::variants($customer->phone_number);
        if ($candidates === []) {
            $candidates = [$customer->phone_number];
        }

        $lastBody = null;
        $lastReason = null;

        foreach ($candidates as $index => $to) {
            $url = 'https://sms.arkesel.com/sms/api'
                .'?action=send-sms'
                .'&api_key='.rawurlencode($apiKey)
                .'&to='.rawurlencode($to)
                .'&from='.rawurlencode($senderId)
            ...
            try {
                ...
                $code = is_array($json) ? strtolower((string) ($json['code'] ?? '')) : '';

                $ok = $response->successful() && ($code === 'ok' || preg_match('/^OK/i', trim($body)));

                if ($ok) {
                    $this->storeVoucherSmsLog($customer, $to, $message, 'sent', $index + 1, null, $body);

                    Log::info('[SMS] Voucher SMS sent via direct Arkesel URL.', [
                        'phone' => $to,
                        'attempt' => $index + 1,
                        'gateway' => $body,
                    ]);
                ...
                $lastReason = 'Gateway reply: '.str($body)->limit(300);
                Log::warning('[SMS] Voucher SMS rejected by gateway.', [
                    'phone' => $to,
                    'attempt' => $index + 1,
                    'gateway' => $body,
                ]);
            } catch (\Throwable $exception) {
                $lastReason = 'Request failed: '.$exception->getMessage();
                Log::error('[SMS] Voucher SMS request failed.', [
                    'phone' => $to,
                    'attempt' => $index + 1,
        ...
        $this->storeVoucherSmsLog($customer, $candidates[0], $message, 'failed', count($candidates), $lastReason, $lastBody);

        return false;
    }

    /**
     * Record the voucher SMS outcome without ever breaking the purchase flow.
     */
    protected function storeVoucherSmsLog(Customer $customer, string $phone, string $message, string $status, int $attempts, ?string $error, ?string $gatewayResponse): void
    {
        try {
            SmsLog::create([
                'customer_id' => $customer->id,
            ...
            Log::error('[SMS] SMS log row could not be saved (run "php artisan migrate" if this says missing column). '.$exception->getMessage());
        }
    }
